...
class ChatTab extends StatefulWidget {
  ...
  const ChatTab({super.key, required this.activityId, required this.members});
...
class _ChatTabState extends State<ChatTab> {
  ...
  Future<void> _sendWithImage() async {
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
      content: Row(children: [
        ...
        SizedBox(width: 12), Text('Envoi de la photo...'),
      ]),
      backgroundColor: _violet, duration: Duration(seconds: 10),
    ));
  ...
  Future<void> _deleteMsg(String id) async {
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      title: const Text('Supprimer ce message ?', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700)),
      actions: [
    ...
    if (ok != true) return;
    await http.delete(Uri.parse('$_baseUrl/api/activities/${widget.activityId}/chat/$id'),
      headers: {'Authorization': 'Bearer ${await _tok()}'});
    _loadChat(silent: true);
  }

  Future<void> _react(String msgId, String emoji) async {
    await http.post(Uri.parse('$_baseUrl/api/activities/${widget.activityId}/chat/$msgId/react'),
  ...
  Future<void> _pinMsg(String msgId) async {
    await http.post(Uri.parse('$_baseUrl/api/activities/${widget.activityId}/chat/$msgId/pin'),
      headers: {'Authorization': 'Bearer ${await _tok()}'});
    _loadChat(silent: true);
  }
  ...
  Widget build(BuildContext context) {
    if (_loading) return const Center(child: CircularProgressIndicator(color: _rose));
    return Container(color: _snow, child: Column(children: [
      ...
      Expanded(child: _messages.isEmpty
        ...
        : ListView.builder(
            ...
            itemCount: _messages.length + (_loadingMore ? 1 : 0) + (_hasMore ? 1 : 0),
            itemBuilder: (_, i) {
              if (_hasMore && i == 0) return _buildLoadMoreBtn();
              if (_loadingMore && i == (_hasMore ? 1 : 0)) return const Padding(
                padding: EdgeInsets.all(8),
                child: Center(child: CircularProgressIndicator(color: _rose, strokeWidth: 2)));
  ...
  Widget _buildOnlineBar() => Container(
    ...
    child: Row(children: [
      ...
      Text('${_onlineUsers.length} en ligne', style: const TextStyle(fontSize: 12, color: _green, fontWeight: FontWeight.w600)),
  ...
  Widget _buildPinnedBar() => Container(
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Message épinglé', style: TextStyle(fontSize: 10, color: _violet, fontWeight: FontWeight.w600)),
        Text(_pinned!['text'] ?? '', style: TextStyle(fontSize: 12, color: _slate.withOpacity(0.8)),
  ...
  Widget _buildEmpty() => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
    Container(padding: const EdgeInsets.all(20),
      decoration: BoxDecoration(color: _rose.withOpacity(0.08), shape: BoxShape.circle),
      child: Icon(Icons.chat_bubble_outline_rounded, size: 40, color: _rose.withOpacity(0.4))),
    const SizedBox(height: 16),
    const Text('Commencez la conversation !', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: _slate)),
    const SizedBox(height: 6),
    Text('Soyez le premier à écrire', style: TextStyle(fontSize: 13, color: _slate.withOpacity(0.5))),
  ]));

  Widget _buildLoadMoreBtn() => TextButton(
    onPressed: _loadMore,
    child: Text('Charger les anciens messages', style: TextStyle(fontSize: 12, color: _slate.withOpacity(0.7))));
  ...
  String _dateLbl(dynamic dateStr) {
    try {
      ...
      return '${d.day} ${['Jan','Fév','Mars','Avr','Mai','Juin','Juil','Août','Sep','Oct','Nov','Déc'][d.month-1]}';
    } catch (_) { return ''; }
  }
  ...
  Widget _buildDateSep(dynamic dateStr) => Padding(
    ...
    child: Row(children: [
      ...
      Container(margin: const EdgeInsets.symmetric(horizontal: 12),
        ...
        child: Text(_dateLbl(dateStr), style: TextStyle(fontSize: 11, color: _slate.withOpacity(0.7), fontWeight: FontWeight.w500))),
      Expanded(child: Divider(color: _border)),
    ]),
  );
    ...
    return GestureDetector(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            if (!isOwn) ...[
              CircleAvatar(radius: 15, backgroundColor: _rose.withOpacity(0.12),
            ...
            Flexible(child: Column(
              ...
              children: [
                if (!isOwn) Padding(padding: const EdgeInsets.only(left: 4, bottom: 3),
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Text(m['username'] ?? '', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 11, color: _ink)),
                    if (_onlineUsers.any((u) => u['username'] == m['username'])) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: deleted ? Colors.grey[100] : isOwn ? _rose : Colors.white,
                    ...
                    boxShadow: deleted ? [] : [BoxShadow(
                      color: (isOwn ? _rose : Colors.black).withOpacity(0.08),
                      blurRadius: 8, offset: const Offset(0, 2))],
                  ),
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    // Épinglé
                    if (pinned) Padding(padding: const EdgeInsets.only(bottom: 5),
                      child: Row(mainAxisSize: MainAxisSize.min, children: [
                        Icon(Icons.push_pin_rounded, size: 11, color: isOwn ? Colors.white60 : _violet),
                        const SizedBox(width: 3),
                        Text('Épinglé', style: TextStyle(fontSize: 9, color: isOwn ? Colors.white60 : _violet, fontWeight: FontWeight.w600)),
                      ])),
                    // Reply
                    if (replyTo != null && replyTo['username'] != null)
                      Container(margin: const EdgeInsets.only(bottom: 7),
                        ...
                        decoration: BoxDecoration(
                          color: isOwn ? Colors.white.withOpacity(0.2) : _rose.withOpacity(0.06),
                          borderRadius: BorderRadius.circular(8),
                          border: Border(left: BorderSide(color: isOwn ? Colors.white54 : _rose, width: 2)),
                        ),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(replyTo['username'], style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700,
                            color: isOwn ? Colors.white70 : _rose)),
                          Text(replyTo['text'] ?? '', style: TextStyle(fontSize: 11,
                            color: isOwn ? Colors.white60 : _slate.withOpacity(0.7)),
                            maxLines: 1, overflow: TextOverflow.ellipsis),
                        ])),
                    // Texte avec mentions colorées
                    if (deleted)
                      Text('🚫 Message supprimé', style: TextStyle(fontSize: 13, color: Colors.grey[400], fontStyle: FontStyle.italic))
                    else
                      _buildTextWithMentions(m['text'] ?? '', isOwn, mentions),
                    // Image
                    if (!deleted && m['imageUrl'] != null && m['imageUrl'].toString().isNotEmpty)
                    ...
                    Row(mainAxisSize: MainAxisSize.min, children: [
                      if (edited) Text('modifié  ', style: TextStyle(fontSize: 9,
                        color: isOwn ? Colors.white54 : _slate.withOpacity(0.4), fontStyle: FontStyle.italic)),
                      Text(timeStr ?? '', style: TextStyle(fontSize: 10,
                        color: isOwn ? Colors.white60 : _slate.withOpacity(0.4))),
                      if (isOwn && readBy > 0) ...[
                        const SizedBox(width: 4),
                ...
                if (reactions.where((r) => r['count'] > 0).isNotEmpty)
                  Padding(padding: const EdgeInsets.only(top: 4),
                    child: Wrap(spacing: 4, children: reactions
                      .where((r) => r['count'] > 0)
                      .map<Widget>((r) => GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: r['reactedByMe'] == true ? _rose.withOpacity(0.1) : Colors.white,
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(color: r['reactedByMe'] == true ? _rose : _border),
                          ),
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            Text(r['emoji'], style: const TextStyle(fontSize: 12)),
                            const SizedBox(width: 3),
                            Text('${r['count']}', style: TextStyle(fontSize: 10,
                              fontWeight: FontWeight.w600,
                              color: r['reactedByMe'] == true ? _rose : _slate)),
                          ])))).toList())),
              ],
            )),
          ],
  ...
  Widget _buildTextWithMentions(String text, bool isOwn, List<String> mentions) {
    if (mentions.isEmpty) {
      return Text(text, style: TextStyle(fontSize: 13, color: isOwn ? Colors.white : _ink, height: 1.4));
    }
    ...
    for (final part in parts) {
      if (part.startsWith('@') && mentions.contains(part.substring(1))) {
        ...
        spans.add(TextSpan(text: part, style: TextStyle(fontSize: 13, color: isOwn ? Colors.white : _ink, height: 1.4)));
      }
    }
  ...
  Widget _buildMentionSuggestions() => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min,
      children: _suggestions.map((u) => ListTile(
        dense: true,
        leading: CircleAvatar(radius: 14, backgroundColor: _rose.withOpacity(0.12),
          child: Text(u[0].toUpperCase(), style: const TextStyle(fontSize: 12, color: _rose, fontWeight: FontWeight.w700))),
        title: Text('@$u', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
        onTap: () {
          final text = _ctrl.text;
  ...
  Widget _buildReplyPreview() => Container(
    ...
    padding: const EdgeInsets.all(10),
    decoration: BoxDecoration(color: _rose.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
    ...
    child: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text('Répondre à ${_replyTo!['username']}',
          style: const TextStyle(fontSize: 11, color: _rose, fontWeight: FontWeight.w600)),
        Text(_replyTo!['text'] ?? '', style: TextStyle(fontSize: 11, color: _slate.withOpacity(0.7)),
          maxLines: 1, overflow: TextOverflow.ellipsis),
      ])),
      GestureDetector(onTap: () => setState(() => _replyTo = null),
        child: Icon(Icons.close_rounded, size: 15, color: _slate.withOpacity(0.5))),
    ]),
  );
  ...
  Widget _buildEditPreview() => Container(
    ...
    decoration: BoxDecoration(color: _violet.withOpacity(0.06), borderRadius: BorderRadius.circular(12),
      border: Border(left: BorderSide(color: _violet, width: 3))),
    child: Row(children: [
      const Icon(Icons.edit_rounded, size: 14, color: _violet),
      const SizedBox(width: 8),
      const Expanded(child: Text('Modification en cours',
        style: TextStyle(fontSize: 12, color: _violet, fontWeight: FontWeight.w600))),
      GestureDetector(onTap: () => setState(() { _editingId = null; _ctrl.clear(); }),
        child: Icon(Icons.close_rounded, size: 15, color: _slate.withOpacity(0.5))),
    ]),
  );
    ...
    child: Row(children: [
      ...
      Expanded(child: TextField(
        ...
        decoration: InputDecoration(
          hintText: _editingId != null ? 'Modifier...' : 'Message... (@username pour mentionner)',
          hintStyle: TextStyle(color: _slate.withOpacity(0.4), fontSize: 12),
      ...
      GestureDetector(
        ...
        child: Container(width: 42, height: 42,
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: _rose.withOpacity(0.35), blurRadius: 8, offset: const Offset(0, 3))]),
          child: Icon(_editingId != null ? Icons.check_rounded : Icons.send_rounded,
  ...
  void _showOptions(Map<String, dynamic> m) {
    ...
    showModalBottomSheet(context: context, backgroundColor: Colors.white,
      ...
      builder: (_) => Padding(padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          _opt(Icons.reply_rounded, 'Répondre', _violet, () {
            ...
            setState(() => _replyTo = {'messageId': msgId, 'username': m['username'], 'text': m['text']});
          }),
          _opt(Icons.push_pin_rounded, 'Épingler', const Color(0xFFF59E0B), () { Navigator.pop(context); _pinMsg(msgId); }),
          if (isOwn) ...[
            _opt(Icons.edit_rounded, 'Modifier', _rose, () {
              Navigator.pop(context);
              setState(() { _editingId = msgId; _ctrl.text = m['text'] ?? ''; });
            }),
            _opt(Icons.delete_outline_rounded, 'Supprimer', Colors.red, () { Navigator.pop(context); _deleteMsg(msgId); }),
          ],
        ])));
  }

  Widget _opt(IconData icon, String label, Color color, VoidCallback onTap) => ListTile(
