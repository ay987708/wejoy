...
class _LoginPageState extends State<LoginPage> with TickerProviderStateMixin {
  ...
  static const List<Color> _neutralGradient = [Color(0xFFAB47BC), Color(0xFFE91E8C)];
  ...
  void _showToast({required String title, required String message, required ToastType type}) =>
  ...
  Future<void> registerUser(String username, String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 201) {
        _showToast(title: "Compte créé !", message: data['message'] ?? "Bienvenue sur WeJoy 🎉", type: ToastType.success);
        setState(() { isLogin = true; _toggleAnimationController.forward(from: 0); });
      } else {
        _showToast(title: "Inscription échouée", message: data['message'] ?? "Une erreur est survenue.", type: ToastType.error);
      }
  ...
  Future<void> forgotPassword() async {
    ...
    if (email.isEmpty || !RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$').hasMatch(email)) {
      _showToast(title: "Email requis", message: "Entrez un email valide avant de continuer.", type: ToastType.warning);
      return;
    }
    ...
    try {
      ...
      ModernLoadingOverlay.hide();
      if (response.statusCode == 200) {
        _showToast(title: "Email envoyé !", message: "Vérifiez votre boîte mail.", type: ToastType.success);
        ...
        _showToast(title: "Erreur", message: data['message'] ?? "Une erreur est survenue.", type: ToastType.error);
      }
    } catch (e) {
      ModernLoadingOverlay.hide();
      _showToast(title: "Erreur", message: "Impossible de contacter le serveur.", type: ToastType.error);
  ...
  Widget _buildTab(String label, bool isActive, VoidCallback onTap) {
    return Expanded(child: GestureDetector(onTap: onTap, child: AnimatedContainer(
      ...
      decoration: BoxDecoration(
        gradient: isActive ? const LinearGradient(colors: _neutralGradient) : null,
      ...
      Align(
        ...
        child: GestureDetector(
          ...
          child: const Padding(
            ...
            child: Text(
              "Mot de passe oublié ?",
              style: TextStyle(color: Color(0xFFAB47BC), fontWeight: FontWeight.w500, decoration: TextDecoration.underline),
            ),
          ),
        ),
      ),
      ...
      return Column(children: [
        Row(children: [
          GestureDetector(
            ...
            child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: const Color(0xFFA855F7).withOpacity(0.1), borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.arrow_back_ios_new_rounded, size: 16, color: Color(0xFFA855F7))),
          ),
          const SizedBox(width: 12),
        ...
        ModernTextField(controller: adminEmailController,    hint: "Email administrateur", icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress, accentColor: const Color(0xFFA855F7)),
        const SizedBox(height: 14),
        ModernTextField(controller: adminPasswordController, hint: "Mot de passe",         icon: Icons.lock_outline,   obscure: adminObscure, toggleObscure: () => setState(() => adminObscure = !adminObscure), accentColor: const Color(0xFFA855F7)),
        const SizedBox(height: 24),
        SizedBox(width: double.infinity, height: 52,
          child: DecoratedBox(
            decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFFA855F7), Color(0xFFEC4899)]), borderRadius: BorderRadius.circular(14)),
    ...
    return Column(children: [
      Row(children: [
        ...
        GestureDetector(
          ...
          child: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: const Color(0xFFA855F7).withOpacity(0.08), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFFA855F7).withOpacity(0.25))), child: const Icon(Icons.admin_panel_settings_outlined, size: 20, color: Color(0xFFA855F7))),
        ),
      ]),
      const SizedBox(height: 24),
      AnimatedSwitcher(
      ...
      AnimatedContainer(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(colors: _neutralGradient),
  ...
  Color get _cardShadow =>
      _mode == 'admin' ? const Color(0xFFA855F7) : Colors.purple.shade100;
            ...
            child: Column(children: [
              ...
              AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  boxShadow: [BoxShadow(color: _cardShadow.withOpacity(0.3), blurRadius: 30, spreadRadius: 5)],
                ),
