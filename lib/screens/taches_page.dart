...
class _TachesPageState extends State<TachesPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _remind(String id, String title) async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(data['message'] ?? 'Rappel envoye !'),
  ...
  void _showCreateCollab() {
    // Capture les couleurs au moment de l'ouverture du dialog
    final rose   = _rose;
          ...
          child: SingleChildScrollView(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                ...
                const Text('Nouvelle tache collaborative',
              ...
              _inputField(titleCtrl, 'Titre de la tache *', Icons.task_alt_rounded, rose),
              const SizedBox(height: 12),
              _inputField(descCtrl, 'Description (optionnel)', Icons.description_outlined, rose, lines: 3),
              const SizedBox(height: 12),
              _inputField(membersCtrl, 'Usernames des membres (separes par virgule)', Icons.people_outline_rounded, rose),
              Padding(
              ...
              GestureDetector(
                  ...
                  final picked = await showDatePicker(
                    ...
                    builder: (_, child) => Theme(
                      data: ThemeData.light().copyWith(
                        colorScheme: ColorScheme.light(primary: rose)),
                      child: child!),
                  );
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _snow,
                    borderRadius: BorderRadius.circular(12),
                    border: Border.all(color: dueDate != null ? rose : _border)),
                  child: Row(children: [
                    Icon(Icons.calendar_today_rounded, size: 18,
                      color: dueDate != null ? rose : _slate.withOpacity(0.5)),
                    const SizedBox(width: 10),
                    Text(
                      dueDate != null
                        ? 'Limite : ${dueDate!.day}/${dueDate!.month}/${dueDate!.year}'
              ...
              Row(children: ['low', 'medium', 'high'].map((p) {
                ...
                final labels = {'low': 'Faible', 'medium': 'Moyenne', 'high': 'Elevee'};
                  ...
                  child: GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: sel ? colors[p]! : _border,
                          width: sel ? 2 : 1)),
                      child: Center(child: Text(labels[p]!,
                        style: TextStyle(
                          fontSize: 12, fontWeight: FontWeight.w600,
                          color: sel ? colors[p]! : _slate)))))));
              }).toList()),
              const SizedBox(height: 20),
                ...
                Expanded(child: ElevatedButton(
                  onPressed: () async {
                    ...
                    if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: const Text('Tache creee et membres notifies'),
                  ...
                  style: ElevatedButton.styleFrom(
                    ...
                    padding: const EdgeInsets.symmetric(vertical: 13),
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
                  child: const Text('Creer', style: TextStyle(fontWeight: FontWeight.w600)))),
              ]),
  ...
  Widget build(BuildContext context) {
    // Ecoute les changements de theme -> rebuild automatique
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Text('Mes Taches',
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _ink)),
                Text('Organisez vos taches personnelles et collaboratives',
  ...
  Widget _buildPersonalTab(Color rose) {
      ...
      child: SingleChildScrollView(
        ...
        child: Column(children: [
            ...
            child: Row(children: [
              Expanded(child: TextField(
                ...
                decoration: InputDecoration(
                  hintText: 'Ajouter une tache personnelle...',
          ...
          if (_personal.isEmpty)
            _buildEmpty('Aucune tache personnelle', 'Ajoutez une tache ci-dessus',
              Icons.task_alt_rounded, rose)
          else
  ...
  Widget _buildCollabTab(Color rose, Color violet) {
    ...
    return RefreshIndicator(
      ...
      child: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (_collab.isEmpty)
            _buildEmpty('Aucune tache collaborative',
              'Creez une tache et invitez vos amis !', Icons.group_outlined, rose),

          if (overdue.isNotEmpty) ...[
            _sectionLabel('En retard', overdue.length, _red),
            const SizedBox(height: 8),
            ...overdue.map((t) => _CollabTaskCard(
              task: t,
              onToggle: () => _toggleComplete(t['id'].toString()),
              onDelete: () => _delete(t['id'].toString()),
              onRemind: () => _remind(t['id'].toString(), t['title']),
          ...
          if (done.isNotEmpty) ...[
            _sectionLabel('Terminees', done.length, _green),
            const SizedBox(height: 8),
            ...done.map((t) => _CollabTaskCard(
              task: t,
              onToggle: () => _toggleComplete(t['id'].toString()),
              onDelete: () => _delete(t['id'].toString()),
              onRemind: () => _remind(t['id'].toString(), t['title']),
  ...
  Widget _buildEmpty(String title, String sub, IconData icon, Color rose) => Padding(
    padding: const EdgeInsets.all(40),
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      Container(
        padding: const EdgeInsets.all(20),
        decoration: BoxDecoration(color: rose.withOpacity(0.07), shape: BoxShape.circle),
        child: Icon(icon, size: 40, color: rose.withOpacity(0.4))),
      const SizedBox(height: 16),
  ...
  Widget _inputField(TextEditingController ctrl, String hint, IconData icon, Color rose, {int lines = 1}) =>
    TextField(
      controller: ctrl,
      maxLines: lines,
      style: const TextStyle(fontSize: 13),
      decoration: InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide(color: rose, width: 2))));
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isOverdue ? _red.withOpacity(0.3) : done ? _green.withOpacity(0.3) : _border),
        boxShadow: [BoxShadow(
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _priorityColor.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Text('Progression', style: TextStyle(
                fontSize: 11, color: _slate.withOpacity(0.7), fontWeight: FontWeight.w500)),
              const Spacer(),
            ...
            ClipRRect(borderRadius: BorderRadius.circular(10),
              child: LinearProgressIndicator(
                ...
                valueColor: AlwaysStoppedAnimation<Color>(
                  task['status'] == 'completed' ? _green : isOverdue ? _red : rose))),
          ]),
        ),
                ...
                child: Row(children: [
                  CircleAvatar(
                    ...
                    child: Text((m['username'] ?? 'U')[0].toUpperCase(),
                      style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: rose))),
                  const SizedBox(width: 10),
                  ...
                  if (memberDone)
                    ...
                    Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                      decoration: BoxDecoration(
                        color: _orange.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
          ...
          Padding(
            ...
            child: Row(children: [
              Icon(Icons.calendar_today_rounded, size: 13,
                color: isOverdue ? _red : _slate.withOpacity(0.5)),
              const SizedBox(width: 6),
              Text('Limite : ${dueDate.day}/${dueDate.month}/${dueDate.year}',
                style: TextStyle(
                  fontSize: 12,
                  color: isOverdue ? _red : _slate.withOpacity(0.7),
              ...
              if (isOverdue) ...[
                ...
                Container(
                  ...
                  child: const Text('EN RETARD',
                    style: TextStyle(fontSize: 9, color: _red, fontWeight: FontWeight.w800))),
              ],
            ]),
          ),
          ...
          child: Row(children: [
            if (task['status'] != 'completed') ...[
              ...
              Expanded(child: ElevatedButton.icon(
                ...
                label: Text(done ? 'Annuler' : 'Marquer fait',
                  style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                style: ElevatedButton.styleFrom(
                  backgroundColor: done ? _slate.withOpacity(0.1) : rose,
                  foregroundColor: done ? _slate : Colors.white,
                  elevation: 0,
                  padding: const EdgeInsets.symmetric(vertical: 9),
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))))),
              const SizedBox(width: 8),
