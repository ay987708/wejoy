...
import 'package:wejoy/screens/defis/games_hub.dart';
...
const _moodStickerLabels = ['Mal', 'Pas bien', 'Pas mal', 'Bien', 'Très bien'];
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  // ── Recommandations IA ──────────────────────────────────────────────────
  bool   _loadingAiRecs = false;
  List<Map<String, dynamic>> _aiRecs = [];
  int    _moodRequestId = 0; // ← token pour annuler les requêtes obsolètes
  ...
  // ══════════════════════════════════════════════════════════════════════
  // Sélection sticker — vide IMMÉDIATEMENT les recs et lance l'IA
  // ══════════════════════════════════════════════════════════════════════
  void _selectMoodSticker(int i) {
    // Arrêter animation du sticker précédent
    ...
    setState(() {
      _journalMoodIndex = i;
      _aiRecs           = [];      // vide immédiatement les anciennes cartes
      _loadingAiRecs    = true;    // affiche le spinner tout de suite
    });
    ...
    _fetchAiRecommendations(i);
  }

  
  Future<void> _fetchAiRecommendations(int moodIndex) async {
    // Chaque appel reçoit un ID unique
    final myRequestId = ++_moodRequestId;

    try {
      final prefs = await SharedPreferences.getInstance();
      final token = prefs.getString('auth_token') ?? '';
      final url   = '$_baseUrl/api/moods/recommend';
      ...
      final response = await http.post(
        ...
        body: jsonEncode({'moodValue': moodIndex + 1}),
      ).timeout(const Duration(seconds: 15));

      // Si l'humeur a changé pendant la requête → ignorer cette réponse
      if (myRequestId != _moodRequestId || !mounted) return;
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _aiRecs        = list;
          _loadingAiRecs = false;
        });
      } else {
        setState(() => _loadingAiRecs = false);
      }
    } catch (e) {
      debugPrint('[AI] Erreur: $e');
      //  Ignorer si l'humeur a changé entre temps
      if (myRequestId != _moodRequestId || !mounted) return;
      setState(() => _loadingAiRecs = false);
    }
  }
      ...
      builder: (_) => Dialog(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: _ink.withOpacity(0.08),
                  ...
                  ? Padding(
                      ...
                      child: Column(mainAxisSize: MainAxisSize.min, children: [
                        Container(
                          ...
                          decoration: BoxDecoration(
                              color: rose.withOpacity(0.06), shape: BoxShape.circle),
                              ...
                              children: [
                                ...
                                Text(n['message'] ?? '',
                                    style: const TextStyle(
                                        fontSize: 12, color: _slate, height: 1.4),
      ...
      RefreshIndicator(
        ...
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _buildPremiumCard(child: WelcomeCard(user: _user, loading: _loadingUser)),
            const SizedBox(height: 18),

            // ── Section Bien-être + Journal ──────────────────────────────
            _buildSectionTitle(
              "Bien-être du moment",
            ...
            // ──  Section dynamique : IA si humeur sélectionnée, sinon centres d'intérêt ──
            if (_journalMoodIndex != -1) ...[
              // Humeur sélectionnée → afficher recs IA
              _buildAiRecommendationSection(),
            ] else ...[
              // Pas d'humeur → afficher recs par centres d'intérêt
              _buildSectionTitle(
                "Recommandé pour toi",
                "Des activités choisies selon tes centres d'intérêt.",
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [BoxShadow(color: _rose.withOpacity(0.30),
  ...
  // ══════════════════════════════════════════════════════════════════════
  //  Section recommandations IA — remplace "Recommandé pour toi"
  // ══════════════════════════════════════════════════════════════════════
  Widget _buildAiRecommendationSection() {
    final moodColor = _moodThemes[_journalMoodIndex]['color'] as Color;
    final moodLabel = _moodStickerLabels[_journalMoodIndex];
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Container(
          ...
          child: Icon(Icons.auto_awesome_rounded, color: moodColor, size: 21)),
        const SizedBox(width: 12),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Activités recommandées\'',
              style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: _ink)),
          Text('Parce que tu te sens « $moodLabel »',
              style: const TextStyle(fontSize: 12.5, color: _slate, height: 1.35)),
        ])),
        // Bouton refresh
        if (!_loadingAiRecs)
          GestureDetector(
            onTap: () => _fetchAiRecommendations(_journalMoodIndex),
            child: Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                  color: moodColor.withOpacity(0.10),
                  borderRadius: BorderRadius.circular(10)),
              child: Icon(Icons.refresh_rounded, color: moodColor, size: 16))),
      ]),
      const SizedBox(height: 12),

      //  Spinner, vide ou cartes — jamais les deux ensemble
      if (_loadingAiRecs)
        _buildAiShimmer(moodColor)
      else if (_aiRecs.isEmpty)
        _buildAiEmpty(moodColor)
      else
        _buildAiCards(moodColor),
    ]);
  }

  Widget _buildAiShimmer(Color color) {
    return SizedBox(
      height: 200,
  ...
  Widget _buildAiEmpty(Color color) {
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          border: Border.all(color: color.withOpacity(0.15))),
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(Icons.explore_off_rounded, size: 36, color: color.withOpacity(0.4)),
        const SizedBox(height: 8),
        Text('Aucune activité trouvée pour cette humeur.',
            style: TextStyle(fontSize: 13, color: _slate.withOpacity(0.7))),
      ]),
    );
  }

  Widget _buildAiCards(Color moodColor) {
    return SizedBox(
      height: 230,
      child: ListView.separated(
        scrollDirection: Axis.horizontal,
        physics: const BouncingScrollPhysics(),
        itemCount: _aiRecs.length,
        separatorBuilder: (_, __) => const SizedBox(width: 12),
        itemBuilder: (_, i) {
          final rec = _aiRecs[i];
          return TweenAnimationBuilder<double>(
            ...
            child: _buildAiActivityCard(rec, moodColor),
          );
        },
      ),
    );
  ...
  Widget _buildAiActivityCard(Map<String, dynamic> rec, Color moodColor) {
    final score    = rec['aiScore']  as int? ?? 0;
    final reason   = rec['aiReason'] as String? ?? '';
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(20),
          border: Border.all(
              color: score >= 70 ? moodColor.withOpacity(0.3) : Colors.transparent,
              width: 1.5),
          boxShadow: [BoxShadow(
              color: moodColor.withOpacity(0.10),
              blurRadius: 16, offset: const Offset(0, 6))],
            ...
            ClipRRect(
              ...
              child: imageUrl != null && imageUrl.isNotEmpty
                  ? Image.network(imageUrl,
                      ...
                      errorBuilder: (_, __, ___) => _aiCardPlaceholder(emoji, moodColor))
                  : _aiCardPlaceholder(emoji, moodColor),
            ),
            // Badge score IA
            Positioned(top: 7, right: 7,
              child: Container(
                ...
                child: Row(mainAxisSize: MainAxisSize.min, children: [
                  Icon(Icons.auto_awesome_rounded, size: 10, color: badgeColor),
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Text(category, style: TextStyle(
                  fontSize: 10, fontWeight: FontWeight.w600, color: moodColor)),
              const SizedBox(height: 3),
  ...
  Widget _aiCardPlaceholder(String emoji, Color color) {
    return Container(
          ...
          child: Column(children: [
            TextField(
              ...
              decoration: InputDecoration(
                ...
                hintStyle: TextStyle(color: _slate.withOpacity(0.5), fontSize: 14),
  ...
  Widget _buildSectionTitle(String title, String subtitle, IconData icon, Color accent) {
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 3),
        Text(subtitle, style: const TextStyle(fontSize: 12.5, color: _slate, height: 1.35)),
      ])),
    ]);
  }
  ...
  Widget _buildPremiumHeader() {
    return Container(
      ...
      child: Row(children: [
        ClipRRect(
          borderRadius: BorderRadius.circular(16),
          child: Image.asset('assets/images/logowejoy.png',
        ...
        const SizedBox(width: 8),
        Stack(children: [
          _premiumHeaderBtn(icon: Icons.notifications_rounded, onTap: _showNotifications),
          if (_notificationCount > 0)
            Positioned(top: 2, right: 2,
              child: Container(
                width: 18, height: 18,
                decoration: BoxDecoration(color: _rose, shape: BoxShape.circle,
  ...
  Widget _premiumHeaderBtn({required IconData icon, required VoidCallback onTap}) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        width: 42, height: 42,
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  gradient: sel ? LinearGradient(colors: [
                    _rose.withOpacity(0.12), _violet.withOpacity(0.10)]) : null,
                  borderRadius: BorderRadius.circular(16)),
                child: Column(mainAxisSize: MainAxisSize.min, children: [
